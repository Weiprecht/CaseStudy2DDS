...
#-------------------------Start Exemplar Appendix---------------------------------------
https://rpubs.com/hanifahpl/emp_att author Ifa
```{r}

# attrition_Data3 <- attrition_Data3[, !names(attrition_Data3) %in% c("Over18", "Over18Num", "EmployeeCount", "StandardHours", "TravelNum")]
# attrition_Data
convert_to_numeric <- function(x) {
  as.numeric(factor(x, levels = unique(x)))
}

ea_cln <- attrition_Data3 %>% 
            mutate(Attrition = ifelse(Attrition == 'Yes', 1, 0),
                   OverTime = ifelse(OverTime == 'Yes', 1, 0),
                   Gender = ifelse(Gender=='Female', 1, 0)) %>% 
...
ea_test %>% 
  dplyr::select(c(Attrition, label_bs, label_sig, label_stp))
...
ea_trn_x <- ea_train %>% dplyr::select(-Attrition)

# Target data train
ea_trn_y <- ea_train[,"Attrition"]

# Predictor data test
ea_tst_x <- ea_test %>% dplyr::select(
...
#-----------------Select and odd number to break ties------------------------
...
ea_test$pred_nv <- predict(object = naive_ea,
                           newdata = ea_test %>% dplyr::select(-c(pred_att_bs, pred_att_sig, pred_att_stp, label_bs, 
                                                          label_sig, label_stp)),
                           type="class") 
...
summary_table <- na.omit(titanic_data) %>%
  mutate(Survived = if_else(Survived == 0, "Died", "Survived")) %>%
  group_by(Pclass, Survived) %>%
  summarise(count = n())

# Create a bar graph
ggplot(data = summary_table, aes(x = factor(Pclass), y = count, fill = factor(Survived))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
  ...
  scale_fill_manual(values = c("Died" = "red", "Survived" = "blue")) +
  theme_minimal()

# Define age bins
age_bins <- c(0, 15, 30, 45, 60, 75, 90)

# Create a summary table of counts with age bins
summary_table2 <- na.omit(titanic_data) %>%
  mutate(AgeGroup = cut(Age, breaks = age_bins),
         Survived = ifelse(Survived == 0, "Died", "Survived")) %>%  # Update Survived column
  group_by(AgeGroup, Survived, Pclass) %>%
  summarise(count = n())

# Create a bar graph
ggplot(data = summary_table2, aes(x = factor(AgeGroup), y = count, fill = factor(Survived))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Survival by Passanger Class and Age",
    x = "Age Ranges",
    y = "Count",
  ...
  scale_fill_manual(values = c("Died" = "red", "Survived" = "blue")) +
  theme_minimal() +
  scale_x_discrete(labels = c("(0,15]", "(15,30]", "(30,45]", "(45,60]", "(60,75]", "(75,90]")) + # Label the x-axis with age bin ranges
  facet_grid(Pclass ~ ., labeller = labeller(Pclass = c("1" = "Class 1", "2" = "Class 2", "3" = "Class 3")))

# Calculate survival rates by Sex
survival_rates <- na.omit(titanic_data) %>%
  mutate(Survived = ifelse(Survived == 0, "Died", "Survived")) %>%
  group_by(Sex, Pclass, Survived) %>%
  summarise(count = n() / nrow(titanic_data))

# Create a bar graph for survival rates by Sex
survival_rates %>%
...
# Create a bar graph
ggplot(data = survival_rates, aes(x = factor(Sex), y = count, fill = factor(Survived))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Survival by Passanger Class and Gender",
    x = "Gender",
    y = "Count",
    fill = "Survival Status"
  ) +
  scale_fill_manual(values = c("Died" = "red", "Survived" = "blue")) +
  theme_minimal() +
  #scale_x_discrete(labels = c("female", "male")) + # Label the x-axis with age bin ranges
  facet_grid(Pclass ~ ., labeller = labeller(Pclass = c("1" = "Class 1", "2" = "Class 2", "3" = "Class 3")))
...
```{r load_aws}
attrition <- read.table(textConnection(getURL(
